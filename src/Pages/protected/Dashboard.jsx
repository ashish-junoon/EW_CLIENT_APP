...
function Dashboard() {
  ...
  // const iskycDone = documents?.others?.some(item => Boolean(item?.other_document_type == "VideoKyc" && item?.status == "True"))
    ...
    onSubmit: async (values) => {
      try {
        ...
        const userRequest = {
          ...
          profile_image_data: selfieCardBase64
            ? selfieCardBase64.split(",")[1]
            : null,
        };
    ...
    try {
      ...
      if (response.status) {
        ...
        navigate("/process-loan", {
          state: { lead_id: userInfo?.lead_id, user_id: userInfo?.user_id },
        });
      } else {
        toast.error(response.message);
      }
  ...
  const handleUploadVideoKyc = async (req) => {
    setIsKyc(true);
    setUserInfo(() => ({
      ...userInfo,
  ...
  return (
    <div className="mb-16">
      ...
      <Card style={"px-4 md:px-8 mb-5"}>
            ...
            <div className="flex justify-end">
              {documents?.aadhaar_pan?.length > 0 && (
                <img
                  ...
                  src={documents?.profileImages?.[0]?.profile_image_code_url || Images.avatar}
                  alt="Profile"
                />
              )}
            </div>
        ...
        <div className="flex justify-center items-center py-3">
          ...
          {userInfo?.is_e_kyc_done === false && (
            <span
              className={`text-xs font-bold px-4 py-1 mx-2 bg-lime-100 text-green-400 rounded shadow-md`}
            >
              In-Process
            </span>
          )}

          {userInfo?.is_e_kyc_done &&
            (userInfo?.lead_status === 5 ||
              userInfo?.lead_status === 24 ||
              userInfo?.lead_status === 25 ||
              userInfo?.lead_status === 26) && (
              <span
                className={`text-xs font-bold px-4 py-1 mx-2 bg-green-100 text-success rounded shadow-md`}
              >
                Approved
              </span>
            )}
          {userInfo?.lead_status === 6 && (
            <span
              className={`text-xs font-bold px-4 py-1 mx-2 bg-green-500 text-white rounded shadow-md`}
            >
              Active
            </span>
          )}
          {userInfo?.lead_status == 10 && (
            <span
              className={`text-xs font-bold px-4 py-1 mx-2 bg-green-200  text-green-500 rounded shadow-md`}
            >
              Closed
            </span>
          )}
        ...
        <div className="grid grid-cols-3 shadow mb-5 py-1.5 px-5 border border-red-200">
          ...
          <div className="col-span-1">
            <div className="flex justify-end">
              <Button
                btnName={
                  loading ? <BtnLoader width={20} height={20} /> : "Verify"
                }
                type={loading ? "button" : "submit"}
                style={`py-1 ${loading ? "bg-gray-500" : "bg-amber-100"} text-amber-600 w-3/4 text-xs font-bold`}
                ...
                <div className="flex justify-center items-center">
                  <div className="">
                    <ImageInput
                      ...
                      aspect={1}
                      onChange={(file) =>
                        selfieUpload.setFieldValue("selfie", file)
                      }
                      onBlur={selfieUpload.handleBlur}
                    />
                    {selfieUpload.touched.selfie &&
                      selfieUpload.errors.selfie && (
                ...
                <div className="col-span-1">
                  <div className="flex justify-center items-center mt-5">
                    <Button
                      btnName={
                        loading ? (
                          <BtnLoader width={20} height={20} />
                        ) : (
                          "Upolad"
                        )
                      }
                      type={loading ? "button" : "submit"}
                      style={`py-1 ${loading ? "bg-gray-500" : "bg-primary"} w-full uppercase text-xs px-4 font-semibold text-white`}
                      disabled={loading}
                    />
                  </div>
                </div>
      ...
      >
        <div className="p-2">
          <p className="text-md font-medium text-red-500">
            ...
            the video camera. Make sure your face and the Aadhaar and Pan card
            are clearly visible. Then, read the text given below loudly and
            clearly, and record the video without any interruption. Ensure the
            video is well-lit and your voice is audible before submitting it.
          </p>
          ...
          <div className="flex justify-end gap-2">
            <Button
              btnName={"Proceed"}
              // type={loading ? "button" : "submit"}
              style={`py-2 px-3 bg-blue-500 text-white text-xs font-bold`}
              onClick={() => {
                (setIsApproved(false), setIsKyc(true));
              }}
              disabled={loading}
            />
            <Button
              btnName={"Cancel"}
              // type={loading ? "button" : "submit"}
              style={`py-2 px-3 text-white bg-yellow-500  text-xs font-bold`}
        ...
        (() => {
          const statusMessages = {
            1: {
              ...
              message:
                "Thank you for choosing us. We are currently processing your application.",
              button: false,
            },
            2: {
              heading: "Under Review",
              message:
                "Your application is being assessed for further consideration.",
              button: false,
            },
            3: {
              heading: "Credit Assessment",
              message:
                "Your application is undergoing a credit assessment to determine eligibility for the credit line.",
              button: false,
            },
            4: {
              heading: "Approved",
            ...
            25: {
              ...
              message:
                "Congratulations! Your application has been Proceed. It will be deposited into your account soon.",
              button: false,
            },
            7: {
              heading: "Rejected",
              message:
                "We regret to inform you that your application has been rejected due to our internal policies.",
              button: false,
            },
            8: {
              heading: "Follow-up",
              message:
                "We have made attempts to contact you but have not yet received confirmation. We will reach out to you again to obtain confirmation.",
              button: false,
            },
            9: {
              heading: "Hold Disbursal",
              message:
                "We regret to inform you that your disbursal has been held due to our internal policies.",
              button: false,
            },
            26: {
              heading: "Amount Disbursement Failed",
              message:
                "Unfortunately, your amount disbursement process was initiated but failed. We will reinitiate it within 24 to 36 hours.",
              button: false,
            },
          };
            ...
            <div className="mb-8 shadow-md rounded-b-lg">
              ...
              {currentStatus && (
                <>
                  ...
                  <div
                    className={`flex flex-col justify-center items-center py-2 border px-5 rounded-b-lg ${
                      userInfo?.is_e_kyc_done
                        ? "border-green-100"
                        : "border-lime-100"
                    }`}
                  >
                    <div className="justify-center">
                      <h6 className="text-sm text-center py-2">
                      ...
                      {currentStatus.button && (
                        <div className="flex justify-center">
                          {(() => {
                            if (
                              !userInfo?.getAssignProduct === null ||
                              ...
                              userInfo?.getAssignProduct?.every(
                                (item) => item.otp_consent_verified === true,
                              ) &&
                              userInfo?.is_e_kyc_done === false
                            ) {
                              return (
                              ...
                              userInfo?.is_e_kyc_done === true &&
                              userInfo?.is_e_nach_activate === false
                            ) {
                              return (
                                <Button
                              ...
                              userInfo?.is_e_nach_activate === true &&
                              userInfo?.is_loan_consent === false
                            ) {
                              return (
                                <Button
      ...
      {userInfo?.lead_status === 6 && (
        <Card style={"mb-4"}>
          <div className="grid grid-cols-2 gap-5">
            <div>
              <h1 className="text-center font-semibold text-xs text-primary">
                Credit Score
              </h1>
              <div className="flex justify-center items-center mt-1">
                <div
                  className={`px-4 py-2 text-white shadow rounded ${
                    userInfo?.cibilCreditScores[0]?.credit_score <= 500
                      ? "bg-danger"
      ...
      {userInfo?.lead_status === 10 && (
        <LoanClosureCard
          customerName={userInfo?.personalInfo[0]?.full_name}
          maxEligibleAmount={loanAmount}
        />
      )}
    </div>
  );
